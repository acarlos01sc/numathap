...
Returns:
    ...
    // Configuration with one or more capabilities

    m.def(
        "configure",
        [](numathap::config::Capability first, py::args rest) {

            numathap::config::MathEnvironment environment;
            numathap::config::Configurator configurator(environment);

            configurator.apply(first);

            for (const auto& arg : rest) {
                configurator.apply(
                    arg.cast<numathap::config::Capability>());
            }
        ...
        R"pbdoc(
Creates a MathEnvironment with one or more capabilities enabled.

Capabilities are applied in the order in which they are supplied.

Args:

    first: The first capability to enable.

    *args: Additional capabilities to enable.

Returns:

    MathEnvironment: A new environment configured with the
    specified capabilities.

Examples:

    env = configure(Capability.Simplify)

    env = configure(
        Capability.ConstantFolder,
        Capability.Simplify,
        Capability.UltraSimplifier
    )
...
``configure(GaussKronrod15Config())`` depending on
``algorithm``.
...
Returns:
    ...
    m.def(
        "configure",
        [](const numathap::backend::integrate::
               AdaptiveSimpsonConfig& config) {
...
Returns:
    ...
    // Configuration with a series type
    ...
    m.def(
        ...
        [](numathap::backend::series::SeriesType type) {
            ...
            throw std::runtime_error(
                "Unknown series algorithm.");
        },

        py::arg("algorithm"));

    // Configuration with Taylor parameters
