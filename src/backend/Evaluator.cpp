...
core::Value Evaluator::evaluateAt(
    const math::PreparedAst& prepared,
    const std::unordered_map<std::string, core::Value>& values) {
    Evaluator evaluator(prepared, values);

    return dispatch::Dispatcher::dispatch(prepared, evaluator);
}

core::Value Evaluator::dispatch(const math::MathNode& node) const {
...
core::Value Evaluator::operator()(const math::UnaryNode& node) const {
...
core::Value Evaluator::operator()(const math::BinaryNode& node) const {
...
core::Value Evaluator::operator()(const math::FunctionNode& node) const {
    std::vector<core::Value> arguments;
    ...
    return prepared_.environment().mathAdapter().callFunction(node.name,
                                                              arguments);
}
