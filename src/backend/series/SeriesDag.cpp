...
SeriesDag::NodeId SeriesDag::intern(NodeKey key) {
    ...
    /*
     * The key was moved into the node above, so it can no longer
     * be used as the hash-consing key. Reconstruct it from the
     * immutable node.
     */
    NodeKey storedKey;
...
SeriesDag::NodeId SeriesDag::importBinary(const math::BinaryNode& node) {
    ...
    return makeBinary(node.op, importNode(*node.left), importNode(*node.right));
}
